        ...
        for y = min(currentData(:,2)):opt.binWidth:max(currentData(:,2))
            ...
            if nVal > 1
                ...
                switch opt.spreadFcn{1}
                    case 'xp'
                        spreadWidth = stdWidth*0.5*(1-exp(log(0.5)*(nVal-1)));
                    case 'lin'
                        spreadWidth = stdWidth*0.5*min(nVal-1,opt.spreadFcn{2})/opt.spreadFcn{2};
                end
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues,m,'+r','Color','r','MarkerSize',z);
            end
        ...
        case 'flipped'
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues,'+r','Color','r','MarkerSize',z);
            end
            if any(opt.showMM==[1,3])
