...
%     end
    ...
    syncs = {'Start','Cue','Poke','Reward','HeadIn'};
    for s = 1:numel(syncs)
        try
        ...
        D(f).(syncs{s}) = nan(ntrials, size(T(f).data.Zscore.Start.FP{1},1));
        D(f).([syncs{s} '_df']) = nan(ntrials, size(T(f).data.Zscore.Start.FP{1},1));
        end
